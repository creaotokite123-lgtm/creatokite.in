...
router.get('/brand', auth, async (req, res) => {
  try {
    ...
    const campaigns = await Campaign.find({ brand: req.user._id, isDeleted: { $ne: true } })
...
router.put('/my/assigned/:id/respond', auth, async (req, res) => {
  try {
    ...
    const campaign = await Campaign.findOneAndUpdate(
      ...
      {
        $set: {
          'assignedCreators.$.status': response,
...
router.put('/my/assigned/:id/submit', auth, async (req, res) => {
  try {
    ...
    const campaign = await Campaign.findOneAndUpdate(
      ...
      {
        $set: {
          'assignedCreators.$.status': 'submitted',
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const finalNiche = Array.isArray(niche) ? niche.join(', ') : (niche || '');
    const finalGoal = Array.isArray(campaignGoal) ? campaignGoal.join(', ') : (campaignGoal || '');
    const finalAudience = Array.isArray(targetAudience) ? targetAudience.join(', ') : (targetAudience || '');

    if (!title || !description || !finalNiche || !deadline)
    ...
    if (isNaN(deadlineDate.getTime()) || deadlineDate < startOfToday) {
      return res.status(400).json({ success: false, message: 'Campaign deadline must be today or a future date' });
    }
    ...
    const campaign = await Campaign.create({
      title, description, niche: finalNiche, budget: finalBudget, deadline: new Date(deadline),
      ...
      isPremium, requiresAdsRights: Boolean(requiresAdsRights), campaignGoal: finalGoal, targetAudience: finalAudience,
      dealType, barterProduct, barterValue: +barterValue || 0, barterDelivery,
      kpiTargets: {
        reach: +kpiTargets.reach || 0, impressions: +kpiTargets.impressions || 0,
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    if (campaign.brand.toString() !== req.user._id.toString() && !['admin', 'superadmin'].includes(req.user.role))
      return res.status(403).json({ success: false, message: 'Not authorized' });

    const allowed = [
      'title', 'description', 'contentGuidelines', 'deadline', 'tags',
      'deliverables', 'platforms', 'targetAudience', 'campaignGoal',
      'requiresAdsRights', 'niche', 'budget', 'totalSlots', 'minFollowers',
      'minEngagement', 'dealType', 'barterProduct', 'barterValue', 'barterDelivery'
    ];

    allowed.forEach(k => {
      if (req.body[k] !== undefined) {
        if (['targetAudience', 'campaignGoal', 'niche'].includes(k) && Array.isArray(req.body[k])) {
          campaign[k] = req.body[k].join(', ');
        } else if (k === 'deadline') {
          const dl = new Date(req.body.deadline);
          const startOfToday = new Date();
          startOfToday.setHours(0, 0, 0, 0);
          if (!isNaN(dl.getTime()) && dl >= startOfToday) {
            campaign.deadline = dl;
          }
        } else if (['budget', 'totalSlots', 'minFollowers', 'minEngagement', 'barterValue'].includes(k)) {
          campaign[k] = +req.body[k] || 0;
        } else {
...
/* DELETE /api/campaigns/:id — brand or admin deletes campaign */
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(campaign.brand, { $inc: { activeCampaignsCount: -1 } });
